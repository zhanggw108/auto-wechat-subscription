...
收回来讲还要允许人类在中间插手，而不是假装 agent 可以一路自动开到终点。
...
这一栏不把新闻复述一遍，只留和论文选题、实验设计、工具复现有关的判断。

- Agent Laboratory 仓库值得看，不是因为它能直接替你产出论文，而是因为它把文献综述、实验、报告写作这条链路做成了一个可观察的工程对象。来源：https://github.com/SamuelSchmidgall/AgentLaboratory
- Anthropic 关于 agent eval 的文章提醒了一件事，评测 agent 不能只看收回来讲答案，还要看任务轨迹、grader 是否可靠，以及哪些节点需要人类复核。来源：https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents
- Anthropic 的 Building Effective Agents 更适合当方法论背景读，它强调先从简单、可组合的 workflow 开始，不要一上来就迷信全自主 agent。来源：https://www.anthropic.com/engineering/building-effective-agents
- OpenAI Evals 仍然适合作为学生搭评测 baseline 的入口，尤其适合学习数据集、grader 和回归式检查怎么组织。来源：https://github.com/openai/evals
- LangGraph 可以作为 agent 工程复现的参考，因为它把状态、持久执行和可控 workflow 放到了比较明确的位置。来源：https://github.com/langchain-ai/langgraph

## 次文章 2：arXiv 高热度文章速报

这一栏不是说下面这些论文都是今天刚发，而是给读者一个相关阅读入口。先知道它们在解决什么问题，再决定要不要深挖。

- Agent Laboratory: Using LLM Agents as Research Assistants，arXiv:2501.04227。这篇适合想做科研 agent、论文助手、实验设计助手的同学读。重点不要只看自动化程度，要看它怎样拆分科研流程，以及人类反馈放在哪些节点。链接：https://arxiv.org/pdf/2501.04227
- Retrieval Augmented Generation or Long-Context LLMs? A Comprehensive Study and Hybrid Approach，arXiv:2407.16833。这篇适合正在纠结长上下文和 RAG 选题的同学读。它的启发不是简单宣布谁淘汰谁，而是把效果、成本和路由选择放在同一个实验框架里讨论。链接：https://arxiv.org/pdf/2407.16833
- AgentBench: Evaluating LLMs as Agents，arXiv:2308.03688。这篇适合做 agent 评测方向的同学读。它把 agent 放进多个交互环境里，能帮助你理解为什么长程推理、指令遵循和失败恢复，比单轮问答更难评。链接：https://arxiv.org/abs/2308.03688
